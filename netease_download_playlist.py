...
def get_url_2_local_file(song_id, url, dist_path, WITH_RENAME=True, song_format="mp3"):
    ...
    if WITH_RENAME:
        dist_name = netease_rename.netease_cache_rename_single(
            song_id, temp_download_path, dist_path, KEEP_SOURCE=False, song_format=song_format
        )
        return dist_name
    else:
        return temp_download_path
...
def netease_songid_2_baidu_single(song_id, strict_level=0):
    ...
    if song_info is not None and "data" not in song_info:
        print(">>>> song info not found, song_id = %s, song_name = %s" % (song_id, song_name))
        return None
    ...
    print(
        ">>>> song_name = %s, artist = %s, song_id = %s, song_id_baidu = %s" % (song_name, song_artist, song_id, song_id_baidu)
    )
...
def baidu_download_single_flac(song_id, dist_path, bit_rate=320000, WITH_RENAME=True):
    ...
    if song_format == "mp3":
        dist_name_temp = get_url_2_local_file(song_id, download_url, dist_path, WITH_RENAME=True, song_format=song_format)
    else:
        dist_name_temp = get_url_2_local_file(song_id, download_url, dist_path, WITH_RENAME=False, song_format=song_format)
...
def netease_download_list(
    song_list, dist_path, bit_rate=320000, WITH_RENAME=True, single_download_func=netease_download_single_bit_rate
):
    song_not_downloaded = []
    song_downloaded = []
...
def download_by_playlist(
    playlist_id, dist_path, bit_rate=320000, WITH_RENAME=True, single_download_func=netease_download_single_bit_rate
):
    play_list = netease_parse_playlist_2_list(playlist_id)
    return netease_download_list(play_list, dist_path, bit_rate, WITH_RENAME, single_download_func)
